...
/test/proc/rotation_test()
	...
	world.log << "0 degrees"
	var/datum/bapi_parsed_map/B = load_map("rotation.dmm", new_z = TRUE, no_changeturf = TRUE)
	if(B.has_warnings())
		CRASH("warnings produced: [json_encode(B.loaded_warnings)]")
	...
	B = load_map("rotation.dmm", orientation = 180, new_z = TRUE, no_changeturf = TRUE)
	if(B.has_warnings())
		CRASH("warnings produced: [json_encode(B.loaded_warnings)]")
	...
	B = load_map("rotation.dmm", orientation = 90, new_z = TRUE, no_changeturf = TRUE)
	if(B.has_warnings())
		CRASH("warnings produced: [json_encode(B.loaded_warnings)]")
	...
	B = load_map("rotation.dmm", orientation = 270, new_z = TRUE, no_changeturf = TRUE)
	if(B.has_warnings())
		CRASH("warnings produced: [json_encode(B.loaded_warnings)]")
