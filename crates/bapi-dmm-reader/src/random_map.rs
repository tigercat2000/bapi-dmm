...
#[byondapi::bind]
pub fn bapidmm_generate_automata(
    limit_x: ByondValue,
    limit_y: ByondValue,
    ...
    Ok(ByondValue::try_from(byond_list.as_slice())?)
}

fn seed_map(limit_x: usize, limit_y: usize, percent_chance: usize) -> eyre::Result<Vec<bool>> {
    let mut map = vec![false; limit_x * limit_y];

    let d = Bernoulli::new(percent_chance as f64 / 100.0)?;
